...
task('deploy-fud-farm-genesis').setAction(async function () {
    ...
    const fudfarmAddress = `0x898291F4D1eFF4AD35d6F0A52F7D85eE12c514B2`
   ...
   /*************** before deployment : *************/
    ...
    const NAME = "FudFarmGenesis";
    const SYMBOL = "FudFarmGenesis"
...
//run withdrawFudFarmGenesis function
    //const runWithdrawFudFarmGenesis = await instance.connect(deployer).withdrawFudFarmGenesis();
   // await runWithdrawFudFarmGenesis.wait();

    console.log(`Deployed FudFarmGenesis to: ${instance.address}`);
    deployments.FudFarmGenesis = instance.address;





    //write FudFarmGenesis to deployments json file 
